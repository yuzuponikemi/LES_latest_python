# -*- coding: utf-8 -*-
"""
Created on Fri Sep 20 14:14:19 2019
...
if shaping == 'inv':
    ...
    versions = ['single_270MPa','single_270MPa_4']
else:
    versions = ['single_130MPa','single_200MPa','single_270MPa']
...
for version in versions:   
    os.chdir('C:/Users/power/Desktop/python/U_VWR_FLOW/momentum4_'+version)
    
    namet = []
    #namelist = glob.glob('./WHFarea*.csv')   #f_UVflow_ave_190806a_002000_0.10ms
    namet = glob.glob('./f_UVflow_ave*.csv')      
...
for case,version in enumerate(versions):
    if period == 'eoi':
        end = enddic[version]+(case)*timelimit-4   # -4 avoid last 0.2ms of tinj
        start = (case)*timelimit + 1
        start = end - int(aveDu/0.05)
    else:
...
if not os.path.exists("C:/Users/power/Desktop/python/images/U_VWRflow/U_VWR_T_HF_"+shaping+length):
    os.mkdir("C:/Users/power/Desktop/python/images/U_VWRflow/U_VWR_T_HF_"+shaping+length)
...
#    plt.xlabel('R', fontsize=30)
    ...
    #plt.title('temp_'+ins.label, fontsize=fontz)
...
#    plt.xlabel('R      mm', fontsize=20)
    plt.ylabel('   delt     mm', fontsize=fontz)
    #plt.title('VWRflow_'+ins.label, fontsize=fontz)
...
#    plt.yticks( np.arange(0, 4.01, 1) )
    ...
    plt.ylabel('   delt     mm', fontsize=fontz)
    #plt.title('Uflow_'+ins.label, fontsize=fontz)
    plt.title('Vertical Flow [1mm--20m/s]', fontsize=fontz)
    #plt.legend(fontsize = 'small',frameon = True,prop={'size':20,})
    plt.xticks( np.arange(0, 20.01,5 ) )
    plt.tick_params(labelsize = fontz,direction = 'in',length = 7)
    
    filename = "C:/Users/power/Desktop/python/images/U_VWRflow/U_VWR_T_HF_"+shaping+length+"/U_VWR_"+ins.label+"ms.png"
